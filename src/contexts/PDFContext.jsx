...
export function PDFProvider({ children }) {
  ...
  const setLineThickness = useCallback((thickness) => {
    dispatch({ type: "SET_LINE_THICKNESS", payload: thickness });
  }, []);
  ...
  const saveAnnotatedPDF = useCallback(
    async (canvasDimensions) => {
      try {
        // Enhanced debugging
        console.log("=== Save PDF Debug Info ===");
        console.log("originalPdfBytes exists:", !!state.originalPdfBytes);
        console.log(
          "originalPdfBytes type:",
          state.originalPdfBytes
            ? state.originalPdfBytes.constructor.name
            : "undefined",
        );
        console.log(
          "originalPdfBytes length:",
          state.originalPdfBytes ? state.originalPdfBytes.length : "N/A",
        );
        console.log("annotations count:", state.annotations.length);

        if (!state.originalPdfBytes || state.originalPdfBytes.length === 0) {
          throw new Error(
            "No PDF data available! Please upload a PDF file first.",
          );
        }

        if (state.annotations.length === 0) {
          throw new Error(
            "No annotations to save! Please add some text annotations first.",
          );
        }
        ...
        const header = String.fromCharCode(
          ...state.originalPdfBytes.slice(0, 5),
        );
        ...
        for (const [pageNum, pageAnnotations] of Object.entries(
          ...
          pageAnnotations.forEach((annotation) => {
            ...
            if (annotation.type === "line") {
              ...
              const x1 =
                (annotation.x1 / annotation.scale) * (width / canvasWidth);
              const y1 =
                height -
                (annotation.y1 / annotation.scale) * (height / canvasHeight);
              const x2 =
                (annotation.x2 / annotation.scale) * (width / canvasWidth);
              const y2 =
                height -
                (annotation.y2 / annotation.scale) * (height / canvasHeight);
              ...
              const pdfX =
                (annotation.x / annotation.scale) * (width / canvasWidth);
              const pdfY =
                height -
                (annotation.y / annotation.scale) * (height / canvasHeight) -
                20;
        ...
        if (error.message.includes("No PDF header found")) {
          errorMessage +=
            "The PDF file appears to be corrupted or invalid. Please try uploading the PDF again.";
        } else if (error.message.includes("PDF-lib library")) {
          errorMessage +=
            "PDF library not loaded. Please refresh the page and try again.";
        } else {
          errorMessage +=
            "Please try again. If the problem persists, try with a different PDF file.";
        }
